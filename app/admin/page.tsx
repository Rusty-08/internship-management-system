  ...
  return (
    <div className="flex h-full flex-col gap-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        >
          <Attendance
            currentAttendance={allInternsAttendance || []}
            isInDashboard
          />
        </DetailsCard>
        <DetailsCard
          ...
          <div className="flex flex-col w-full border shadow-sm rounded-md overflow-hidden">
            {allInterns.map((user, idx) => (
              <Fragment key={user.id}>
                <Link
                  href={`/admin/intern-management/profile/${user.internId}`}
                >
                  <div
                    className={cn(
                  ...
                  >
                    <div className="flex items-center gap-2">
                      <Avatar className="w-7 h-7">
                        <AvatarImage
                          src={user.internImage || undefined}
                          alt={`${user.intern}`}
                        />
                        <AvatarFallback>
                          <Image
                      ...
                      <div className="flex flex-col">
                        <p className="text-sm group-hover:text-primary font-medium transition-colors">
                          {user.intern}
                        </p>
                    ...
                    <Badge
                      variant={
                        haveOngoingTask(user.tasks).length > 0
                          ? 'IN_PROGRESS'
                          : 'secondary'
                      }
                      className="hidden lg:inline-flex flex-shrink-0"
                    >
                      {haveOngoingTask(user.tasks).length}
                      {' ongoing '}
                      {haveOngoingTask(user.tasks).length > 1
                        ? 'tasks'
                        : 'task'}
                    </Badge>
                  </div>
