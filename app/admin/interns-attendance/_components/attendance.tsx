...
import { AttendanceProps, attendanceColumns } from '@/app/intern/my-attendance/_components/attendance-columns'
...
const Attendance = ({ currentAttendance, isInDashboard = false }: AdminAttendanceProps) => {
  const [date, setDate] = useState<Date | undefined>(new Date())
  
  const filteredAttendance = date ? (currentAttendance.filter(
    attendance => format(attendance.date, 'PPP') === format(date, 'PPP')
  )) : []
  ...
  return (
    <div className="flex flex-col gap-4">
      {!isInDashboard && <DayPicker date={date || undefined} setDate={setDate} />}
      <AttendanceTable
